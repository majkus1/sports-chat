...
export function normalizePick({ market, selection, homeName, awayName }) {
	...
	if (has(m, 'gole druzyny', 'team goals', 'gole ')) {
	...
	// --- Suma goli ---
	if (has(m, 'suma goli', 'total goals', 'goals over', 'over/under', 'liczba goli')) {
		const line = readLine(s);
	...
	// --- Zwycięzca meczu ---
	if (has(m, 'wynik meczu', 'zwyciezca', '1x2', 'match winner', 'full time result', 'final result')) {
