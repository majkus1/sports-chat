 ...
 *
 * ROZSTRZYGAMY PO SILE TRAFIENIA, NIE PO SAMEJ OBECNOŚCI. Poprzednia wersja pytała tylko,
 * czy nazwa się pojawia, i poddawała się, gdy pasowały obie — a pasowały zawsze, gdy
 * drużyny dzielą człon nazwy. „Manchester City powyżej 0.5 gola" w meczu z Manchesterem
 * United trafiało w obie strony przez samo słowo „manchester", więc typ szedł do kosza
 * jako nierozpoznany. Tak samo Real Madryt z Realem Sociedad, Bayer z Bayernem czy
 * dowolne dwie drużyny z tego samego miasta.
 *
 * Pełna nazwa waży więcej niż pojedyncze słowo, a dłuższe słowo więcej niż krótsze, więc
 * „manchester city" (trafienie pełne) wygrywa z „manchester" (trafienie częściowe).
 * Remis nadal daje `null`: przy prawdziwej dwuznaczności lepszy typ pominięty niż zgadnięty.
 */
function whichTeam(text, homeName, awayName) {
	const t = norm(text);

	const sila = (nazwa) => {
		const pelna = norm(nazwa);
		if (!pelna) return 0;
		if (t.includes(pelna)) return pelna.length;
		// Pierwszy człon nazwy wystarcza — model skraca „Deportivo Pasto" do „Pasto".
		let best = 0;
		for (const slowo of pelna.split(' ')) {
			if (slowo.length > 3 && t.includes(slowo)) best = Math.max(best, slowo.length);
		}
		return best;
	};

	const home = sila(homeName);
	const away = sila(awayName);
...
/**
 * SIEDEM SELEKCJI, KTÓRE WYSTAWIAMY — z nazwą rynku, postacią znormalizowaną i tekstem.
 *
 * Tekst selekcji ma jednocześnie CZYTAĆ SIĘ PO LUDZKU i WRACAĆ przez `normalizePick` do tej
 * samej postaci znormalizowanej. To nie jest kosmetyka: model językowy przepisuje `selection`
 * znak w znak (inaczej typ przepadał), więc cokolwiek tu napiszemy, trafia i do oczu
 * czytelnika, i z powrotem do parsera.
 *
 * Raport miał własną, trzecią kopię tych nazw — i to się zemściło dokładnie tak, jak
 * ostrzega komentarz przy `CANONICAL_MARKETS`. Wystawiał „Wynik meczu: away" (kod zamiast
 * drużyny, brzydkie, ale rozpoznawalne) oraz „Gole drużyny: Gość powyżej 0.5", którego
 * parser nie miał jak rozpoznać: gałąź goli drużyny szuka NAZWY ZESPOŁU, a nie słowa
 * „gość". Każdy taki typ czytelnik widział jak każdy inny, a statystyka go nie liczyła.
 *
 * Kolejność jest kolejnością prezentacji. Każdy `label` jest tak zbudowany, żeby parser
 * rozpoznał go po kotwicy odpornej na nazwy drużyn — „(gospodarze)", „(goście)", „1X",
 * „X2" — zanim w ogóle sięgnie po dopasowanie nazwy.
 *
 * UWAGA PRZY ZMIANIE BRZMIENIA: TEKST SELEKCJI WCHODZI DO KLUCZA UNIKALNOŚCI TYPU.
 * Indeks w `models/Pick.js` to `{ fixtureId, kind, market, selection, userId }`, i to on
 * sprawia, że ponowne wygenerowanie tej samej analizy nie zapisuje typu drugi raz. Zmiana
 * samego brzmienia sprawia, że ten sam typ przestaje być tym samym rekordem: przy pierwszej
 * zmianie tych opisów cztery typy z raportu zapisały się po raz drugi i weszłyby do
 * skuteczności podwójnie. Jeżeli zmieniasz tu tekst, policz duplikaty po `normalized`
 * i zostaw po jednym rekordzie na selekcję.
 */
export const SELECTION_SHAPES = {
	home: {
		market: CANONICAL_MARKETS.matchWinner,
		normalized: { type: 'matchWinner', value: 'home' },
		label: (n) => `${n?.homeName || 'Gospodarze'} (gospodarze)`,
	},
	away: {
		market: CANONICAL_MARKETS.matchWinner,
		normalized: { type: 'matchWinner', value: 'away' },
		label: (n) => `${n?.awayName || 'Goście'} (goście)`,
	},
	'1X': {
		market: CANONICAL_MARKETS.doubleChance,
		normalized: { type: 'doubleChance', value: '1X' },
		label: (n) => `1X (${n?.homeName || 'gospodarz'} lub remis)`,
	},
	X2: {
		market: CANONICAL_MARKETS.doubleChance,
		normalized: { type: 'doubleChance', value: 'X2' },
		label: (n) => `X2 (${n?.awayName || 'gość'} lub remis)`,
	},
	homeScores: {
		market: CANONICAL_MARKETS.teamGoals,
		normalized: { type: 'teamGoals', side: 'home', dir: 'over', line: 0.5 },
		label: (n) => `${n?.homeName || 'Gospodarze'} powyżej 0.5 gola`,
	},
	awayScores: {
		market: CANONICAL_MARKETS.teamGoals,
		normalized: { type: 'teamGoals', side: 'away', dir: 'over', line: 0.5 },
		label: (n) => `${n?.awayName || 'Goście'} powyżej 0.5 gola`,
	},
	/*
	 * Jedyny rynek sumy goli — i tylko „powyżej". Nie z macierzy Dixona-Colesa, tylko po
	 * kalibracji cechami drużyn (`lib/model/goals.js`); norma jest ligowa i przychodzi
	 * z typem (`baseRate`), nie z tabeli — patrz `baseRateFor` w polityce.
	 */
	over25: {
		market: CANONICAL_MARKETS.totalGoals,
		normalized: { type: 'totalGoals', dir: 'over', line: 2.5 },
...
export function normalizePick({ market, selection, homeName, awayName }) {
	...
	if (has(m, 'gole druzyny', 'team goals', 'gole ', 'druzyna strzeli', 'strzeli gol', 'to score', 'strzelone gole')) {
		/*
		 * Strona po nazwie zespołu, a gdy jej nie ma — po słowie wskazującym stronę.
		 *
		 * Sama nazwa nie wystarczała: gdy pakiet meczu nie niósł nazw drużyn, selekcja
		 * brzmiała „Gospodarze powyżej 0.5 gola" i przepadała, choć jest jednoznaczna.
		 * Gałąź wyniku meczu rozumiała „gospodarz"/„gość" od zawsze; ta nie, i ta różnica
		 * kosztowała typy. Kolejność ma znaczenie — nazwa zespołu jest mocniejsza,
		 * bo słowo „gość" mogłoby stać w zdaniu obok nazwy przeciwnika.
		 */
		let team = whichTeam(both, homeName, awayName);
		if (!team) {
			if (/\bhome\b/.test(s) || has(s, 'gospodarz')) team = 'home';
			else if (/\baway\b/.test(s) || has(s, 'gosc')) team = 'away';
		}
