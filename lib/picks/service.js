...
import { normalizePick, settlePick, sameSelection } from '@/lib/picks/markets';
import { MIN_PLAYED } from '@/lib/picks/policy';
...
/*
 * PRÓG ROZEGRANYCH MECZÓW JEST WSPÓLNY Z RAPORTEM — patrz `MIN_PLAYED` w polityce typów.
 * Osobna stała tutaj (6 wobec 4 w raporcie) przez miesiąc wycinała z publicznej statystyki
 * prawie połowę pokazywanych typów. Nie wraca.
 */
 ...
 */
export function qualifiesForStats({ dataQuality, sectionsPresent, playedHome, playedAway }) {
	...
	if (Number.isFinite(playedHome) && playedHome < MIN_PLAYED) return false;
	if (Number.isFinite(playedAway) && playedAway < MIN_PLAYED) return false;
	return true;
}

/** Rodzaje, w obrębie których ta sama selekcja na ten sam mecz jest jedną prognozą. */
const DEDUPE_GROUP = { report: ['report', 'prematch'], prematch: ['report', 'prematch'], live: ['live'] };

/**
 * Wcześniejszy zapis tej samej selekcji na ten sam mecz — albo `null`.
 *
 * Porównanie po postaci znormalizowanej (`sameSelection`), nie po tekście, bo to właśnie
 * tekst się rozjeżdża. Własny rekord (ten sam klucz unikalności) nie jest oryginałem dla
 * samego siebie — ponowne wygenerowanie tej samej analizy ma być cichym no-op, jak dotąd.
 */
async function findOriginal({ fixtureId, kind, normalized, pick, userId }) {
	const grupa = DEDUPE_GROUP[kind];
	if (!grupa) return null;
	const kandydaci = await Pick.find({
		...
		.select('_id kind market selection userId normalized createdAt')
		.sort({ createdAt: 1 })
		.lean();
	const tenSamKlucz = (k) =>
		k.kind === kind &&
		k.market === String(pick.market || '') &&
		k.selection === String(pick.selection || '') &&
		String(k.userId ?? '') === String(userId ?? '');
	return kandydaci.find((k) => !tenSamKlucz(k) && sameSelection(k.normalized, normalized)) || null;
}
...
export async function recordPicks({
	...
	for (const pick of picks) {
		try {
			...
			/*
			 * JEDNA PROGNOZA NA JEDNO ZDARZENIE LICZY SIĘ RAZ.
			 *
			 * Raport z czwartku i analiza z soboty potrafią wystawić tę samą selekcję na ten sam
			 * mecz; to samo dwie wersje brzmienia jednej selekcji. Klucz unikalności rozróżnia
			 * rodzaj i tekst, więc powstają dwa rekordy — i do września 2026 oba wchodziły do
			 * skuteczności: 12 ze 123 rozliczonych typów było policzonych podwójnie. Teraz drugi
			 * zapis wskazuje pierwszy i nie liczy się; pierwszy zostaje, bo był opublikowany
			 * wcześniej i nie da się go wybrać po fakcie.
			 *
			 * Typ na żywo to inna prognoza niż przedmeczowa (inny stan meczu, inna norma), więc
			 * grupa dedupe dla „live" jest osobna. Typy do Kolejki są poza statystyką i tak.
			 */
			const original = normalized ? await findOriginal({ fixtureId: fixtureCtx.fixtureId, kind, normalized, pick, userId }) : null;
			...
			await Pick.updateOne(
				...
				},
				{
					$setOnInsert: {
						...
						homeId: Number.isFinite(fixtureCtx.homeId) ? fixtureCtx.homeId : null,
						awayId: Number.isFinite(fixtureCtx.awayId) ? fixtureCtx.awayId : null,
						...
						// Zapisane, bo decydują o `countsToStats` — bez nich nie da się potem sprawdzić, dlaczego typ nie wszedł.
						playedHome: Number.isFinite(meta.playedHome) ? meta.playedHome : null,
						playedAway: Number.isFinite(meta.playedAway) ? meta.playedAway : null,
						/*
						 ...
						 */
						/*
						 * `excludeFromStats` — jawne wyłączenie z publicznej skuteczności, gdy typ
						 * powstaje w innym celu niż analiza. Typy modelu do Kolejki są takim przypadkiem:
						 * ten sam mecz i ta sama selekcja mogą już liczyć się z raportu, a klucz
						 * unikalności rozróżnia rodzaje, więc bez tej flagi trafienie liczyłoby się
						 * podwójnie. Ranking kolejki ma własny licznik i tam typ jest widoczny.
						 */
						countsToStats:
							!meta.excludeFromStats &&
							!original &&
							qualifiesForStats(meta) &&
							(polityka.ok || countsAsFallback(polityka.reason)),
						duplicateOf: original ? original._id : null,
						policyReason: polityka.reason,
						roundKey: meta.roundKey ?? null,
