...
export async function recordPicks({
	...
	for (const pick of picks) {
		try {
			 ...
			 */
			const norma = Number.isFinite(pick.baseRate) ? { base: pick.baseRate } : {};
			const polityka = meetsPolicy(normalized, probability, norma);
			const { base, lift } = liftFor(normalized, probability, norma);
			...
			await Pick.updateOne(
				...
				},
				{
					$setOnInsert: {
						...
						/*
						 * Dwa warunki, oba muszą być spełnione: dane musiały wystarczać do prognozy,
						 * a sam typ musi mieścić się w rynkach i progach potwierdzonych pomiarem.
						 *
						 * Drugi warunek jest tu, a nie w prompcie, bo prompt to prośba. Analiza meczu
						 * nie miała dotąd żadnego filtru po stronie kodu — jedyną barierą była instrukcja,
						 * której model językowy może nie posłuchać.
						 */
						countsToStats: qualifiesForStats(meta) && polityka.ok,
