...
import { over25For } from '@/lib/model/goals';
 ...
 *
 * `over25` to wynik `over25For` z `lib/model/goals.js` (ułamki) — liczony przez wywołującego,
 * bo wymaga bazy i terminarza, a ta funkcja ma zostać czysta. Bez niego selekcji „powyżej
 * 2,5" po prostu nie ma; w meczu w trakcie też nie, bo kalibracja była mierzona przed meczem.
 *
 * @param {{ leagueModel: object|null, fixture: object, implied?: object|null, over25?: object|null }} input
 * @returns {null | { version, matchesUsed, inPlay, minute, score, probabilities, likeliestScore, selections }}
 */
export function buildAnalysisModel({ leagueModel, fixture, implied = null, over25 = null }) {
...
/**
 * Skalibrowane „powyżej 2,5" dla meczu przed pierwszym gwizdkiem; `null` w trakcie, bez
 * modelu albo bez historii drużyn. Nigdy nie rzuca — to dodatek, nie warunek analizy.
 */
export async function over25ForFixture(leagueModel, fixture) {
...
export async function modelForBundle(bundle) {
	...
	try {
		...
		const over25 = await over25ForFixture(leagueModel, fixture);
		return buildAnalysisModel({ leagueModel, fixture, implied: bundle.market ?? null, over25 });
	} catch (error) {
