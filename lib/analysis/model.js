...
import { baseRateFor, liftFor, meetsPolicy } from '@/lib/picks/policy';
 ...
 *
 * @param {{ leagueModel: object|null, fixture: object }} input
 * @returns {null | { version, matchesUsed, inPlay, minute, score, probabilities, likeliestScore, selections }}
 */
export function buildAnalysisModel({ leagueModel, fixture }) {
	...
	const selections = SELEKCJE.map((s) => {
		...
		const polityka = meetsPolicy(s.normalized, probability, { base });
		return {
			key: s.key,
			market: s.market,
...
export async function modelForBundle(bundle) {
	...
	try {
		...
		return buildAnalysisModel({ leagueModel, fixture });
	} catch (error) {
