...
/**
 * Średnia bramkowa z rozbiciem na dom i wyjazd, ze zjazdem na wartość łączną.
 *
 * Dostawca podaje `average` jako `{home, away, total}`, a my braliśmy wyłącznie `total` —
 * czyli oceniali gospodarza jego wynikami z wyjazdów, a gościa jego wynikami u siebie.
 * Przewaga własnego boiska to najsilniejszy pojedynczy efekt w piłce; uśrednianie jej
 * kosztowało dokładnie te rynki, które wypadały najgorzej (progi goli: 30% trafień).
 *
 * Zjazd na `total` jest konieczny: na starcie sezonu albo w pucharze drużyna może nie mieć
 * jeszcze meczu w danej roli, a wtedy split jest zerem, nie brakiem danych.
 */
function sideAverage(form, kierunek, side) {
	const avg = form?.goals?.[kierunek]?.average;
	if (!avg) return null;
	const rozegrane = form?.played?.[side];
	// Mniej niż dwa mecze w danej roli to jeszcze nie przesłanka — bierzemy wartość łączną.
	if (Number.isFinite(rozegrane) && rozegrane >= 2 && avg[side] != null) return avg[side];
	return avg.total ?? null;
}
...
export function evaluateMarkets({ prediction, formHome, formAway }) {
	...
	// Atak gospodarza U SIEBIE przeciw defensywie gościa NA WYJEŹDZIE — i odwrotnie.
	const lambdaHome = expectedGoals(
		sideAverage(formHome, 'for', 'home'),
		sideAverage(formAway, 'against', 'away')
	);
	const lambdaAway = expectedGoals(
		sideAverage(formAway, 'for', 'away'),
		sideAverage(formHome, 'against', 'home')
	);
