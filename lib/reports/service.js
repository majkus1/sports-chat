...
import { liftFor, meetsPolicy, marketProbabilityFor, MARKET_CEILING, MIN_PLAYED } from '@/lib/picks/policy';
import { normalizePick, sameSelection, SELECTION_SHAPES } from '@/lib/picks/markets';
 ...
 *
 * Stąd trzy zabezpieczenia poniżej: `playedByTeam` odcina ligi na starcie sezonu, zanim
 * wydamy na nie choć jedno wywołanie, `modelCanYield` odsiewa mecze, w których własny model
 * i tak nie wystawi żadnej selekcji, a `allocateBudget` pilnuje, żeby żadna liga nie zabrała
 * całej puli i żeby rozgrywki bez modelu nie wypadły z raportu w całości.
 */
const PREDICTIONS_BUDGET = 40;

/**
 * Ile miejsc w budżecie rezerwujemy dla rozgrywek, w których własny model się nie dopasował.
 *
 * `modelCanYield` potrafi ocenić mecz z góry tylko tam, gdzie model ligi istnieje. W pucharach
 * i w rozgrywkach z krótką historią prawdopodobieństwo powstaje dopiero z prognozy dostawcy,
 * czyli PO wydaniu zapytania — takiego meczu nie da się odsiać wcześniej ani docenić.
 * Bez tej rezerwy odsiew wyciąłby je co do jednego i raport zwęziłby się do lig z modelem,
 * co jest dokładnie tym zawężeniem, które właśnie naprawiamy o poziom wyżej.
 */
const UNMODELLED_SLOTS = 10;
 ...
 */
export const MAX_PROBABILITY = 92;
...
/*
 * NAZW RYNKÓW ANI OPISÓW SELEKCJI TU JUŻ NIE MA — RAPORT PYTA O NIE `lib/picks/markets`.
 *
 * Ten plik miał własną kopię: nazwy rynków po polsku i selekcje w postaci `'home'`,
 * `'away'`, `'Gość powyżej 0.5'`. Model językowy przepisuje `selection` znak w znak, więc
 * te kody trafiały wprost przed oczy czytelnika („Wynik meczu: away") — a w rynku goli
 * drużyny także do parseru, który szuka tam NAZWY ZESPOŁU, nie słowa „gość". Typ nie dawał
 * się rozpoznać, więc wypadał ze statystyki, wyglądając przy tym jak każdy inny.
 *
 * `SELECTION_SHAPES` trzyma jednocześnie postać znormalizowaną i tekst czytelny dla
 * człowieka, zbudowany tak, by parser rozpoznał go z powrotem. Ta sama stała rządzi
 * analizą pojedynczego meczu, więc typ z raportu i typ z analizy nazywają się tak samo.
 */

/*
 * `MIN_PLAYED` przychodzi z polityki typów — ta sama liczba decyduje, czy typ powstaje
 * i czy się liczy. Własna kopia tutaj (4) rozjechała się kiedyś z progiem statystyki (6);
 * szczegóły przy stałej.
 */
 ...
 * sił drużyn, wypadło lepiej; wszystko, co zależy od SUMY goli Z SAMEJ MACIERZY — gorzej:
 *
 ...
 * gorszy niż jego brak. Gole drużyny wiszą na granicy — przy większej próbie ich przewaga
 * stopniała do szumu.
 *
 * WYJĄTEK OD WRZEŚNIA 2026: „powyżej 2,5 gola" wraca, ale nie z macierzy — po kalibracji
 * regresją z cechami drużyn (`lib/model/goals.js`). Eksperyment na 19 692 meczach: model
 * przegrywał ze stałą z nadmiaru pewności, nie z braku informacji; po kalibracji Brier
 * 0,2436 wobec 0,2479, t = 6,5, a typy po polityce trafiają w 72–75 %. Innych progów
 * i „obie strzelą" nadal nie ma.
 *
...
export function evaluateMarkets({
	...
	const out = [];
	const nazwy = { homeName, awayName };
	/*
	 * `klucz` wskazuje selekcję w `SELECTION_SHAPES`. Wpis niesie normę rynku i przewagę
	 * nad nią — to po nich sortujemy i to pokazujemy czytelnikowi, nie sam procent.
	 */
	const consider = (klucz, pModel, { support = 1, base: normaLigowa } = {}) => {
		...
		const ksztalt = SELECTION_SHAPES[klucz];
		const normalized = ksztalt.normalized;
		// Norma ligowa (suma goli) zastępuje tabelaryczną — tak samo w polityce i w przewadze.
		const opcje = Number.isFinite(normaLigowa) ? { base: Math.round(normaLigowa) } : {};
		if (!meetsPolicy(normalized, p, opcje).ok) return;
		const { base, lift } = liftFor(normalized, p, opcje);
		out.push({
			market: ksztalt.market,
			selection: ksztalt.label(nazwy),
			normalized,
			pModel: p,
			base,
			lift,
	...
	if (pModelu) {
		consider('home', pModelu.home, { support: zgodne(pModelu.home, pct.home) });
		consider('away', pModelu.away, { support: zgodne(pModelu.away, pct.away) });
		...
		consider('1X', pModelu.dc1X, { support: zgodne(pModelu.dc1X, dostawcaDC1X) });
		consider('X2', pModelu.dcX2, { support: zgodne(pModelu.dcX2, dostawcaDCX2) });

		// Gole drużyny: czy dana drużyna w ogóle strzeli.
		consider('homeScores', pModelu.homeScores);
		consider('awayScores', pModelu.awayScores);

		/*
		 * Suma goli — tylko „powyżej 2,5" i tylko skalibrowane (`lib/model/goals.js`).
		 * Z macierzy ten rynek przegrywał ze stałą; po kalibracji cechami drużyn bije ją
		 * istotnie (eksperyment w README modelu). Norma jest ligowa, więc idzie z typem.
		 */
		if (m.over25 && Number.isFinite(m.over25.probability)) {
			consider('over25', 100 * m.over25.probability, { base: 100 * m.over25.base });
		}
...
/** Daty (w czasie polskim) pokrywające okno od teraz do `hours` godzin w przód. */
function datesInWindow(hours, now = Date.now()) {
	return localDatesBetween(now, now + hours * 3600_000);
}
...
/**
 * Czy z tej prognozy modelu w ogóle wyjdzie jakakolwiek selekcja.
 *
 * TO NIE JEST HEURYSTYKA, TYLKO TEN SAM RACHUNEK CO NIŻEJ. Pytamy `evaluateMarkets` bez
 * prognozy dostawcy — ta w gałęzi modelu służy wyłącznie do premii za zgodność i nie ma
 * wpływu na to, czy selekcja przechodzi próg. Odpowiedź jest więc DOKŁADNIE tym, co ta sama
 * funkcja policzy po pobraniu prognozy: pusta lista teraz znaczy pustą listę potem.
 *
 * Dlatego to filtr, a nie ranking, i dlatego jest bezpieczny. Sortowanie 152 meczów po
 * własnej ocenie i branie czubka wciągałoby na górę nie tylko spotkania naprawdę odstające,
 * ale i te, w których model myli się w korzystną stronę — a wiemy z pomiaru, że przy typach
 * bywa około czterech punktów zbyt pewny siebie. Odrzucanie dołu tej wady nie ma: najgorsze,
 * co grozi po pomyłce, to jeden mecz mniej w puli, a nie typ wystawiony z powodu błędu.
 *
 * @param {object|null} modelPrediction wynik `predictFixture`
 * @returns {boolean|null} `null`, gdy modelu ligi nie ma i nie da się nic orzec
 */
export function modelCanYield(modelPrediction) {
	if (!modelPrediction) return null;
	return evaluateMarkets({ modelPrediction }).length > 0;
}

/**
 * Dzieli budżet prognoz między mecze ocenione z góry a te, których ocenić się nie da.
 *
 * Grupa bez modelu ma gwarantowaną, ale ograniczoną liczbę miejsc; niewykorzystane miejsca
 * jednej grupy przechodzą do drugiej, więc budżet nigdy nie marnuje się przez sam podział.
 * Wewnątrz każdej grupy kolejność ustala `spreadAcrossLeagues`, czyli ranga i różnorodność.
 *
 * @param {{ pewne: Array, niepewne: Array, budget: number, reserved: number, tierOf: Function }} o
 */
export function allocateBudget({ pewne, niepewne, budget, reserved, tierOf }) {
	const kolejkaPewnych = spreadAcrossLeagues(pewne, { budget, tierOf });
	const kolejkaNiepewnych = spreadAcrossLeagues(niepewne, { budget, tierOf });

	const dlaNiepewnych = Math.min(kolejkaNiepewnych.length, reserved);
	const dlaPewnych = Math.min(kolejkaPewnych.length, budget - dlaNiepewnych);
	// Gdy ocenionych meczów jest mniej, niż mogłyby zająć, resztę oddajemy nieocenionym.
	const uzupelnienie = Math.min(kolejkaNiepewnych.length, budget - dlaPewnych);

	return [...kolejkaPewnych.slice(0, dlaPewnych), ...kolejkaNiepewnych.slice(0, uzupelnienie)];
}
...
export async function buildReportCandidates({ type }) {
	...
	/*
	 * 2c. Mecze, w których własny model i tak nic nie wystawi, odpadają przed zapytaniem.
	 *
	 * `predictFixture` nie kosztuje wywołania na mecz: model dopasowuje się raz na ligę
	 * i siedzi w pamięci procesu, a terminarz bieżącego sezonu mamy już z kroku 2b. Cała
	 * dopłata to drugi sezon do dopasowania, czyli jedno wywołanie na rozgrywki.
	 *
	 * Dotąd te 40 zapytań szło na mecze wybrane po randze ligi i godzinie, więc większość
	 * trafiała w spotkania, z których nic nie mogło wyjść: 11 kandydatów z 40 obejrzanych.
	 * Teraz zapytanie dostaje mecz, który ma czym zaowocować.
	 */
	const zModelem = [];
	const bezModelu = [];
	// Prognoza zapamiętana, nie liczona drugi raz: to macierz wyników na mecz, a niżej
	// potrzebuje jej dokładnie ten sam zestaw spotkań.
	const prognozy = new Map();
	for (const f of zProba) {
		...
		prognozy.set(f.id, modelPrediction);
		const rokuje = modelCanYield(modelPrediction);
		if (rokuje === null) bezModelu.push(f);
		else if (rokuje) zModelem.push(f);
	}

	// 2d. Budżet rozłożony po ligach, z gwarantowaną częścią dla rozgrywek bez modelu.
	const doPrognoz = allocateBudget({
		pewne: zModelem,
		niepewne: bezModelu,
		budget: PREDICTIONS_BUDGET,
		reserved: UNMODELLED_SLOTS,
	...
	for (let i = 0; i < doPrognoz.length; i += 10) {
		...
		for (const { fixture, raw } of results) {
			...
			/*
			 * Prognoza policzona już w kroku 2c — tam decydowała, czy w ogóle pytać o ten
			 * mecz. Model dopasowuje się raz na ligę i siedzi w pamięci procesu, więc nawet
			 * powtórne wywołanie nie kosztowałoby zapytania; chodzi o macierz wyników, której
			 * nie ma po co składać drugi raz dla tego samego spotkania.
			 */
			const modelPrediction = prognozy.get(fixture.id) ?? null;

			const wstepne = evaluateMarkets({
				prediction,
				formHome,
				formAway,
				modelPrediction,
