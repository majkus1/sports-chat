...
/**
 * Terminarz dnia — DNIA W CZASIE POLSKIM, nie w UTC.
 *
 * Bez `timezone` dostawca dzieli doby po UTC: mecz o 1:00 w nocy czasu polskiego lądował
 * w poprzednim dniu, a lista „dziś" pokazywała spotkania z jutra. Z parametrem dzień
 * zaczyna się i kończy tam, gdzie u użytkownika; `fixture.date` przychodzi wtedy
 * z przesunięciem strefy w zapisie, co `Date.parse` rozumie bez zmian.
 */
export function fixturesByDate(date) {
	return cachedFetch({
		namespace: 'fixtures',
		params: { date, timezone: APP_TIMEZONE },
		ttlSeconds: TTL.fixturesByDate,
		fetcher: () => footballRequest('fixtures', { date, timezone: APP_TIMEZONE }),
	});
}
