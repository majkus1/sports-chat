...
export function buildMorningEmail({ name, locale, date, yesterday, today, favorites, round, siteUrl, unsubscribeUrl }) {
	...
	T.push(`${t.assistant} ${siteUrl}/${lang}/pilka-nozna/asystent`, '');
	T.push(t.disclaimer, '', t.why, `${t.unsubscribe}: ${unsubscribeUrl}`);
	const text = T.join('\n');
	...
	H.push(`<p style="margin:18px 0 0 0;color:#888;font-size:13px;line-height:1.6;">${esc(t.disclaimer)}</p>`);
