	...
	for (const user of users) {
		try {
			...
			try {
				...
				/*
				 * Nieudana wysyłka zdejmuje znacznik. Bez tego pierwszy błąd (np. odrzucone
				 * hasło SMTP) blokowałby ponowną próbę do końca dnia — a przecież nic nie doszło.
				 * Znacznik przed wysyłką chroni przed DWOMA mailami; tu nie było żadnego.
				 */
				await User.updateOne({ _id: user._id }, { $set: { 'morningEmail.lastSentOn': null } });
				throw error;
			}
			summary.sent += 1;
		} catch (error) {
