...
/** YYYY-MM-DD w czasie polskim. */
export function warsawDate(d = new Date()) {
	return new Intl.DateTimeFormat('en-CA', { timeZone: TZ }).format(d);
}

/** Północ danego dnia w czasie polskim, jako `Date` (UTC). */
export function warsawMidnight(dateStr) {
	const utcMidnight = new Date(`${dateStr}T00:00:00Z`);
	const hour = Number(
		new Intl.DateTimeFormat('en-US', { timeZone: TZ, hour: '2-digit', hour12: false }).format(utcMidnight)
	);
	// O północy UTC w Warszawie jest 1:00 albo 2:00 — to jest przesunięcie strefy tego dnia.
	return new Date(utcMidnight.getTime() - (hour % 24) * 3600_000);
}

function kickoffLocal(iso) {
	return new Intl.DateTimeFormat('pl-PL', { timeZone: TZ, hour: '2-digit', minute: '2-digit' }).format(new Date(iso));
}

function dayBefore(dateStr) {
	return new Intl.DateTimeFormat('en-CA', { timeZone: 'UTC' }).format(new Date(Date.parse(`${dateStr}T12:00:00Z`) - 86_400_000));
}
...
export async function sendMorningEmails({ now = new Date(), dryRun = false } = {}) {
	...
	const todayStart = warsawMidnight(today);
	const tomorrowStart = new Date(todayStart.getTime() + 86_400_000);
	const yesterdayStart = warsawMidnight(yesterday);
