 ...
 * (`goals.js`). Regresja zapisana w `goalsCalibrationData.js` ma sens tylko z DOKŁADNIE tym
...
/**
 * Odniesienia ligowe z okna ostatnich meczów: częstość „powyżej 2,5" (i „obie strzelą"),
 * średnie gole i strzały NA DRUŻYNĘ w meczu.
 *
...
export function leagueReference(matches) {
	...
	return {
		...
		// BTTS: tylko do kafelka „obie strzelą" w analizie — typów w tym rynku nie ma.
		bttsRate: srednia(dane.map((x) => (x.hg > 0 && x.ag > 0 ? 1 : 0))),
		gf: para(sr('hg'), sr('ag')),
