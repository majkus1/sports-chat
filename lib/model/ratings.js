...
export function fitRatings(matches, { referenceDate, xi = DEFAULT_XI, rho = DEFAULT_RHO } = {}) {
	...
	for (let iter = 0; iter < ITERATIONS; iter += 1) {
		...
		for (const m of przygotowane) {
			...
			/*
			 * Pochodna log-wiarogodności Poissona po parametrze w wykładniku to po prostu
			 * (obserwacja − oczekiwanie), przemnożona przez wagę meczu. Stąd tak krótki zapis.
			 */
		...
		const skala = LEARNING_RATE / przygotowane.length;
		for (let i = 0; i < teamIds.length; i += 1) {
			attack[i] += skala * (gAttack[i] - RIDGE * attack[i]);
			defence[i] += skala * (gDefence[i] - RIDGE * defence[i]);
		}
		homeAdvantage += skala * gHome;
		intercept += skala * gIntercept;
