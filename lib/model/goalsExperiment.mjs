...
function stalaLigi(leagueId, kiedy) {
	...
	if (!ref) return null;
	// BTTS ma własną stałą — reszta odniesień (gole, strzały) jest wspólna.
	return { ...ref, over25: ref.rate, btts: srednia(dane.map((x) => x.btts)) };
}
 ...
 */
if (EMIT) {
	const y = uczace.map((u) => u.over25);
	const variants = {};
	for (const [wariant, klucz] of [['goals', 'D'], ['shots', 'E']]) {
		const params = fitLogistic(uczace.map((u) => u.X.over25[klucz]), y);
		variants[wariant] = { features: FEATURE_NAMES[wariant], ...params };
	}
	const zapis = {
		market: 'totalGoals over 2.5',
	...
	const naglowek =
		'/* PLIK GENEROWANY — nie edytować ręcznie. Regresja rynku „powyżej 2,5 gola";\n' +
