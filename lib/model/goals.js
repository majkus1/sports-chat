...
/**
 * Skalibrowane prawdopodobieństwo „powyżej 2,5 gola" dla pary drużyn.
 *
 * @param {{ leagueId: number, season: number, homeId: number, awayId: number, dcOver25: number }} input
 *   `dcOver25` — ułamek z Dixona-Colesa (`predictMarkets(...).totalGoals[2.5].over`)
 * @returns {Promise<null | { probability: number, base: number, variant: 'goals'|'shots' }>}
 *   ułamki 0–1; `base` to stała ligowa — norma, wobec której liczy się przewaga typu
 */
export async function over25For({ leagueId, season, homeId, awayId, dcOver25 }) {
	...
	const params = calibration?.variants?.[variant];
	const x = featureVector(variant, { league: ctx.league, dcOver: dcOver25, home, away });
	const probability = applyCalibration(params, x);
	if (!Number.isFinite(probability)) return null;

	return { probability, base: ctx.league.rate, variant };
}
