...
import { recordForecasts } from '@/lib/model/forecastLog';
 ...
 *
 * `onPrediction` dostaje każdą prognozę (także bez plakietki) — do dziennika prognoz,
 * który mierzy model na wszystkim, co policzył, nie tylko na tym, co przeszło próg.
 *
 * @param {Array} fixtures znormalizowane mecze (`normalizeFixture`)
 * @param {{ onPrediction?: (fixture: object, prediction: object, hint: object|null) => void }} [options]
 * @returns {Promise<Map<string, object>>}
 */
export async function modelHintsFor(fixtures, { onPrediction } = {}) {
	const out = new Map();
	for (const f of fixtures || []) {
		...
		onPrediction?.(f, modelPrediction, hint);
	}
	return out;
}
...
export async function hintsForDate(date) {
	...
	/*
	 * Dziennik prognoz — w tle, bez czekania. Lista ma się pokazać niezależnie od bazy;
	 * niezapisana prognoza to strata jednego pomiaru, nie awaria.
	 */
	const doDziennika = [];
	const byId = await modelHintsFor(fixtures, {
		onPrediction: (fixture, prediction, hint) => doDziennika.push({ fixture, prediction, hint }),
	});
	if (doDziennika.length) {
		recordForecasts(doDziennika).catch((error) =>
			console.warn('[forecast-log] zapis prognoz nie powiódł się:', error.message)
		);
	}
	const list = fixtures
