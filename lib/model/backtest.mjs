...
const { medianMatchesPerTeam, MIN_MATCHES, MIN_MATCHES_PER_TEAM, EXCLUDED_COMPETITIONS } =
	await import('@/lib/model');
...
/*
 * MODEL DOPASOWYWANY OSOBNO DLA KAŻDYCH ROZGRYWEK — TAK JAK NA PRODUKCJI.
 *
 * Pierwsza wersja backtestu uczyła JEDEN model na wszystkich ligach naraz i dobierała `rho`
 * na danych uczących. Produkcja (`getLeagueModel`) robi co innego: osobne dopasowanie dla
 * każdych rozgrywek, z domyślnym `rho`. Mierzyliśmy więc model, którego nie wdrażamy —
 * a różnica nie jest kosmetyczna. Model globalny widzi drużynę pucharową przez pryzmat jej
 * meczów ligowych; model per rozgrywki zna wyłącznie te dwa spotkania w pucharze.
 *
 * Progi wejścia i lista wykluczeń pochodzą z `lib/model`, nie z kopii w tym pliku, żeby
 * narzędzie pomiarowe nie mogło rozjechać się z produkcją przy pierwszej zmianie.
 */
/**
 ...
 *
 * MUSI STAĆ PRZED `modelDlaLigi`. Stała `const` w module ma martwą strefę czasową, więc
 * funkcja zdefiniowana wyżej i wywołana przed tą linią wywraca się na `Cannot access
 * before initialization` — kolejność deklaracji jest tu częścią poprawności, nie stylem.
 */
const REFIT_DAYS = Number(arg('refit-days', '14'));

const modeleLig = new Map();
let dopasowan = 0;

/** Czy rozgrywki spełniają warunki produkcji na dzień `kiedy` — bez dopasowywania modelu. */
function dopuszczone(mecze, leagueId) {
	return (
		!EXCLUDED_COMPETITIONS.has(leagueId) &&
		mecze.length >= MIN_MATCHES &&
		medianMatchesPerTeam(mecze) >= MIN_MATCHES_PER_TEAM
	);
}

/** Mecze rozgrywek rozegrane PRZED chwilą `kiedy` — inaczej model znałby odpowiedź. */
function doNauki(leagueId, kiedy) {
	return wszystkie.filter((x) => x.leagueId === leagueId && new Date(x.date).getTime() < kiedy);
}

function modelDlaLigi(leagueId, kiedy) {
	const wpis = modeleLig.get(leagueId);
	if (wpis && kiedy < wpis.waznyDo) return wpis.model;

	const mecze = doNauki(leagueId, kiedy);
	const waznyDo = kiedy + Math.max(REFIT_DAYS, 1) * 86_400_000;

	if (!dopuszczone(mecze, leagueId)) {
		modeleLig.set(leagueId, { model: null, waznyDo });
		return null;
	}

	// Bez `rho` w opcjach — produkcja też go nie dobiera, korzysta z wartości domyślnej.
	const model = fitRatings(mecze, { referenceDate: new Date(kiedy) });
	dopasowan += 1;
	modeleLig.set(leagueId, { model, waznyDo });
	return model;
}

console.log('\nModel dopasowywany osobno dla każdych rozgrywek, tak jak na produkcji.');
{
	// Sam warunek wejścia, bez dopasowywania — inaczej płacilibyśmy za 41 modeli tylko po to,
	// żeby je zaraz wyrzucić i policzyć od nowa w pętli testowej.
	const ligiWTescie = [...new Set(testowe.map((m) => m.leagueId))];
	const zModelem = ligiWTescie.filter((id) => dopuszczone(doNauki(id, SPLIT_DATE.getTime()), id));
	const odrzucone = ligiWTescie.filter((id) => !zModelem.includes(id));
	const nazwa = (id) => testowe.find((m) => m.leagueId === id)?.leagueName || String(id);

	/*
	 * To jest MIGAWKA NA DZIEŃ PODZIAŁU, nie stan końcowy. Rozgrywki, które wtedy nie miały
	 * jeszcze dość meczów, mogą dostać model w trakcie sezonu — w pętli testowej warunek
	 * jest sprawdzany na bieżąco. Bez tego zastrzeżenia wiersz myli: przy poprzednim
	 * przebiegu Liga Europy figurowała tu jako „bez modelu", a niżej miała 81 ocenionych meczów.
	 */
	console.log(
		`  rozgrywki w puli testowej: ${ligiWTescie.length} | z modelem na dzień podziału: ${zModelem.length} | jeszcze bez modelu: ${odrzucone.length}`
	);
	if (odrzucone.length) {
		console.log(`  bez modelu na dzień podziału (mogą go dostać w trakcie sezonu): ${odrzucone.map(nazwa).join(', ')}`);
	}
}
...
/** Mecze, których produkcja w ogóle nie obejmie modelem — poza pomiarem, nie w nim. */
let bezModelu = 0;

for (const m of testowe) {
	/*
	 * Model dla rozgrywek tego meczu, uczony WYŁĄCZNIE na spotkaniach wcześniejszych.
	 * Gdyby do danych uczących wpadł choćby jeden mecz z tego samego dnia, model znałby
	 * część odpowiedzi i cały backtest przestałby cokolwiek znaczyć.
	 */
	const kiedy = new Date(m.date).getTime();
	const model = modelDlaLigi(m.leagueId, kiedy);

	/*
	 * Brak modelu to nie porażka, tylko inna ścieżka: produkcja użyje wtedy prognoz dostawcy
	 * albo modelu językowego. Takie mecze wypadają z CAŁEGO pomiaru, razem z liniami
	 * odniesienia — inaczej porównywalibyśmy model i częstości na różnych zbiorach.
	 */
	if (!model) {
		bezModelu += 1;
		continue;
...
/*
 * Mianownik to mecze OBJĘTE MODELEM, nie wszystkie testowe. Spotkania, których produkcja
 * nie obejmie, nie mogą wchodzić do średniej — ani na plus, ani na minus.
 */
const n = testowe.length - bezModelu;
if (n < 100) {
	console.log(`\nModel objął tylko ${n} meczów testowych — za mało na wiarygodny wynik.`);
	process.exit(1);
}
console.log(
	`  meczów testowych: ${testowe.length} | objętych modelem: ${n} (${((100 * n) / testowe.length).toFixed(1)}%) | poza modelem: ${bezModelu} | z nieznaną drużyną: ${nieznane} (${((100 * nieznane) / n).toFixed(1)}%) | dopasowań: ${dopasowan}`
);
console.log('');
...
if (rynek.n) {
	...
	kalibracja.forEach((k, i) => {
		...
		/*
		 * Odczyt porównuje model z FAKTEM, nie z rynkiem.
		 *
		 * Pierwsza wersja mierzyła odległość modelu od rynku i nazywała ją błędem — czyli
		 * traktowała rynek jak prawdę. Rynek prawdą nie jest, jest tylko mocną prognozą.
		 * W kubełku 80–89% dawało to „model przeszacowany o 7 pkt" w sytuacji, gdy model
		 * podał 84% i tyle właśnie wyszło, a to RYNEK mówił 77%. Kto jest bliżej faktu,
		 * pokazujemy osobno.
		 */
		const bladModelu = sm - sf;
		const bladRynku = sr2 - sf;
		const odczyt =
			Math.abs(bladModelu) < 0.02
				? `trafiony${Math.abs(bladRynku) >= 0.04 ? ' (rynek dalej od faktu)' : ''}`
				: `${bladModelu > 0 ? 'przeszacowany' : 'niedoszacowany'} o ${(100 * Math.abs(bladModelu)).toFixed(0)} pkt` +
					(Math.abs(bladRynku) < Math.abs(bladModelu) - 0.02 ? ', rynek bliżej' : '');
		console.log(
			'    ' +
...
console.log(
	'    ' + 'próg'.padEnd(8) + 'typów'.padStart(8) + 'meczów'.padStart(9) + 'deklar.'.padStart(9) + 'trafność'.padStart(10) + 'norma'.padStart(9) + 'PRZEWAGA'.padStart(11) + '   po sufcie'
);
...
for (const prog of [0, 6, 9, 12, 15, 18, 21, 25]) {
	...
	/*
	 * Ile MECZÓW dostaje choć jeden typ — nie ile typów powstaje.
	 *
	 * To dwie różne liczby i myli je łatwo. Przy progu 12 wychodzi mniej więcej jeden typ
	 * na mecz, ale rozkładają się nierówno: spotkanie z wyraźnym faworytem generuje trzy
	 * naraz (wynik, podwójna szansa, gol), a mecz dwóch słabych drużyn żadnego. Dla
	 * czytelnika liczy się właśnie to drugie — analiza bez typu wygląda na pustą.
	 */
	const zTypem = new Set(wybrane.map((k) => k.fixtureId)).size;

	console.log(
		'    ' +
			`${prog} pkt`.padEnd(8) +
			String(wybrane.length).padStart(8) +
			`${((100 * zTypem) / n).toFixed(0)}%`.padStart(9) +
