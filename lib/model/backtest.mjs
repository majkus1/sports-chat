...
function modelDlaLigi(leagueId, kiedy) {
	...
	// Wyłącznie mecze rozegrane PRZED tym spotkaniem — inaczej model znałby odpowiedź.
	const mecze = wszystkie.filter(
		(x) => x.leagueId === leagueId && new Date(x.date).getTime() < kiedy
	);
...
console.log('\nModel dopasowywany osobno dla każdych rozgrywek, tak jak na produkcji.');
{
	...
	const dopuszczone = ligiWTescie.filter((id) => modelDlaLigi(id, SPLIT_DATE.getTime()));
	console.log(
		`  rozgrywki w puli testowej: ${ligiWTescie.length} | z modelem: ${dopuszczone.length} | odrzucone progiem lub listą: ${ligiWTescie.length - dopuszczone.length}`
	);
	modeleLig.clear();
	dopasowan = 0;
}
