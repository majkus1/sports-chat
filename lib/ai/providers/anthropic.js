...
/**
 * Wyszukiwanie w internecie — serwerowe narzędzie `web_search`.
 *
 * Wersja narzędzia zależy od modelu: nowsze modele mają wariant z filtrowaniem wyników,
 * starsze tylko podstawowy. Cytowania przychodzą przy blokach tekstu, nie osobno.
 * Opłata za wyszukanie doliczana do tokenów jak u drugiego dostawcy.
 */
const WEB_SEARCH_USD = 0.01;

function webSearchToolFor(model) {
	const nowszy = /(opus-5|sonnet-5|fable-5|opus-4-[678]|sonnet-4-6)/.test(String(model || ''));
	return { type: nowszy ? 'web_search_20260209' : 'web_search_20250305', name: 'web_search', max_uses: 3 };
}

/** Tekst i źródła z odpowiedzi — czysta funkcja, do testów. */
export function extractSearchResult(response) {
	...
	for (const block of response?.content || []) {
		...
		for (const c of block.citations || []) {
			if (c.url && !zrodla.has(c.url)) zrodla.set(c.url, { title: c.title || c.url, url: c.url });
		}
	}
	const searches = response?.usage?.server_tool_use?.web_search_requests ?? 0;
	return { text: text.trim(), sources: [...zrodla.values()], searches };
}

export async function searchWeb({ model, prompt, maxTokens = 1200 }) {
	const response = await getClient().messages.create({
		model,
		max_tokens: maxTokens,
		tools: [webSearchToolFor(model)],
