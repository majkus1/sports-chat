...
import { CANONICAL_MARKETS } from '@/lib/picks/markets';
import { formatModelSection } from '@/lib/analysis/model';

export const PROMPT_VERSION = 'match-analysis/15';
...
const PROGI_TYPOW = [
	...
	.concat([
		// Norma sumy goli jest ligowa i przychodzi z sekcją modelu — tu tylko orientacyjnie.
		'   - powyżej 2,5 gola: norma ligowa (zwykle 48–57%), WYŁĄCZNIE z sekcji PROGNOZA MODELU LICZBOWEGO',
	])
	.join('\n');
...
8. SUMY GOLI I "OBIE STRZELĄ" NIE TYPUJESZ Z WŁASNEJ GŁOWY. Żadnego "poniżej X gola", żadnego "powyżej 1.5/3.5", żadnego "obie drużyny strzelą". Powód jest zmierzony: na 8715 meczach prognoza z samych statystyk wypada w tych rynkach GORZEJ niż stałe zgadywanie średniej ligowej. JEDYNY wyjątek: "${CANONICAL_MARKETS.totalGoals}" / "Powyżej 2.5 gola" — i tylko wtedy, gdy stoi w sekcji PROGNOZA MODELU LICZBOWEGO jako KANDYDAT NA TYP, z liczbą przepisaną z tej sekcji. Ta selekcja jest liczona osobnym, skalibrowanym rachunkiem z golami i strzałami obu drużyn; bez tej sekcji nie istnieje. Możesz omówić potencjał bramkowy w "summary" i "keyFactors" — ale nie zamieniaj tego na typ. Pole "goals" (oczekiwana suma, over25, btts) wypełniaj dalej: gdy sekcja PROGNOZA MODELU LICZBOWEGO podaje wiersz „Gole", przepisz z niego te trzy liczby co do znaku; bez tej sekcji wpisz własną ocenę — to orientacyjny opis charakteru meczu.
9. POLE "market" WYPEŁNIAJ DOKŁADNIE JEDNĄ Z CZTERECH NAZW, przepisaną znak w znak, bez tłumaczenia i bez zmiany szyku: "${CANONICAL_MARKETS.matchWinner}", "${CANONICAL_MARKETS.doubleChance}", "${CANONICAL_MARKETS.teamGoals}", "${CANONICAL_MARKETS.totalGoals}". Inna nazwa sprawia, że typ NIE ZOSTANIE ROZPOZNANY i przepada — czytelnik zobaczy go bez prawdopodobieństwa, a statystyka nigdy go nie policzy. Nie wymyślaj własnych nazw w rodzaju "Drużyna strzeli gola" ani "Zwycięzca meczu". Format pola "selection": dla "${CANONICAL_MARKETS.matchWinner}" — nazwa drużyny albo "Remis"; dla "${CANONICAL_MARKETS.doubleChance}" — "1X" albo "X2"; dla "${CANONICAL_MARKETS.teamGoals}" — "NAZWA DRUŻYNY powyżej 0.5 gola"; dla "${CANONICAL_MARKETS.totalGoals}" — wyłącznie "Powyżej 2.5 gola" i wyłącznie z sekcji PROGNOZA MODELU LICZBOWEGO (punkt 8). Tylko te rynki mają potwierdzoną przewagę nad zgadywaniem.
...
${PROGI_TYPOW}
   "Gość strzeli gola" przy 80% to nie typ, tylko opis tego, co dzieje się w większości meczów, a "1X" przy wyraźnym faworycie gospodarzy nie mówi czytelnikowi nic, czego sam by nie wiedział.
   TYP ZAPASOWY, GDY NIC NIE SIĘGA PROGU: jeśli żadna selekcja nie spełnia obu warunków, mimo to wystaw DOKŁADNIE JEDEN typ — ten, który całość danych wspiera najmocniej. Analiza bez żadnego typu jest dla czytelnika bezużyteczna, więc wolimy typ słabszy niż żaden. W "rationale" napisz wprost, że dane nie dają wyraźnej przewagi i na czym opierasz wskazanie. Nie naciągaj przy tym "probability" do progu — podaj liczbę, w którą naprawdę wierzysz.
