 ...
 * + przewaga nad wyceną rynku). Model dostaje gotową krótką listę z danymi selekcji
 * i pełnymi faktami — jego rola to weryfikacja każdego kandydata na tle formy i H2H,
 * wybór najlepiej wspartego rynku oraz zwięzła analiza. Nie szuka meczów sam.
 *
 * Zasada nadrzędna przeniesiona ze starego agenta (ai_agent/ai_analyzer.py): dane
 * o wycenie rynkowej służą WYŁĄCZNIE selekcji — w treści raportu nie wolno wspomnieć
 * o kursach, bukmacherach ani jakiejkolwiek "opłacalności".
 */
...
ZAKAZ ABSOLUTNY:
1. W żadnym polu odpowiedzi nie wolno wspomnieć o kursach, bukmacherach, zakładach pieniężnych, marżach, "value", wycenie rynkowej ani prawdopodobieństwie implikowanym. Pole "przewaga selekcji" w danych to informacja wewnętrzna — wykorzystaj ją do oceny, nigdy do cytowania. Nie zachęcaj do gry na pieniądze.
...
function formatMarket(entry) {
	return `${entry.market} / ${entry.selection} — wstępne prawdopodobieństwo ${entry.pModel}%, przewaga selekcji +${entry.edge} p.p.`;
}
