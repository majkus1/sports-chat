...
function formatTeamForm(form, side) {
	 ...
	 * ROZBICIE DOM / WYJAZD.
	 *
	 * Dostawca zwraca komplet średnich osobno dla meczów u siebie i na wyjeździe, ale przez
	 * długi czas pokazywaliśmy modelowi wyłącznie wartości łączne. To najkosztowniejsza
	 * strata informacji w całym prompcie: drużyna ze średnią 1,6 gola potrafi mieć 2,3
	 * u siebie i 0,9 na wyjeździe, a analizowany mecz rozgrywa się w jednym z tych dwóch
	 * światów, nie w ich średniej. Dane były pobierane i wyrzucane.
	 */
	const gdzie = side === 'home' ? 'u siebie' : 'na wyjeździe';
	const splitKey = side === 'home' ? 'home' : 'away';
	const rozegraneTam = form.played?.[splitKey];

	const rozklad = form.goals?.for?.overUnder;
	const progi = rozklad
		? Object.entries(rozklad)
				.map(([prog, v]) => `${prog}: ${v?.over ?? '?'} powyżej / ${v?.under ?? '?'} poniżej`)
				.join(', ')
		: null;
	...
	return [
		...
		line('śr. gole zdobyte (łącznie)', form.goals?.for?.average?.total),
		line('śr. gole stracone (łącznie)', form.goals?.against?.average?.total),

		// Wiersze rozstrzygające — model ma oceniać drużynę w warunkach TEGO meczu.
		rozegraneTam
			? line(
					`MECZE ${gdzie.toUpperCase()} (${rozegraneTam})`,
					`bilans ${form.wins?.[splitKey] ?? '?'}Z / ${form.draws?.[splitKey] ?? '?'}R / ${form.loses?.[splitKey] ?? '?'}P`
				)
			: null,
		line(`śr. gole zdobyte ${gdzie}`, form.goals?.for?.average?.[splitKey]),
		line(`śr. gole stracone ${gdzie}`, form.goals?.against?.average?.[splitKey]),
		line(`czyste konta ${gdzie}`, form.cleanSheet?.[splitKey]),
		line(`mecze bez gola ${gdzie}`, form.failedToScore?.[splitKey]),
		...
		line('czyste konta (łącznie)', form.cleanSheet?.total),
		line('mecze bez gola (łącznie)', form.failedToScore?.total),

		// Rozkład meczów wg progów bramkowych — liczy się przy typach na sumę goli
		// bardziej niż sama średnia, bo pokazuje rozrzut, a nie tylko środek.
		progi ? line('mecze wg progu goli strzelonych', progi) : null,
...
/**
 * Kiedy padają gole i w jakich ustawieniach gra drużyna.
 *
 * Rozkład minutowy odpowiada na pytanie, którego nie da się zadać średniej: czy zespół
 * rozstrzyga mecze przed przerwą, czy dowozi je w końcówce. Dwie drużyny z tą samą średnią
 * 1,6 gola to przy typie na sumę bramek dwa różne mecze.
 */
function formatTeamStats(stats, teamName) {
	if (!stats) return null;

	const wiersze = [`  ${teamName}:`];

	const rozklad = (blok, etykieta) => {
		if (!blok?.length) return;
		const opis = blok
			.map((m) => `${m.zakres} min: ${m.goals}${m.percentage ? ` (${m.percentage})` : ''}`)
			.join(', ');
		wiersze.push(`    ${etykieta}: ${opis}`);
	};

	rozklad(stats.goalsByMinute?.for, 'gole zdobyte wg minuty');
	rozklad(stats.goalsByMinute?.against, 'gole stracone wg minuty');

	if (stats.streak) {
		wiersze.push(
			`    najdłuższe serie w sezonie: ${stats.streak.wins ?? 0} zwycięstw, ${stats.streak.draws ?? 0} remisów, ${stats.streak.loses ?? 0} porażek`
		);
	}

	if (stats.formations?.length) {
		const ustawienia = stats.formations
			.slice(0, 3)
			.map((f) => `${f.formation}${f.played ? ` (${f.played} m)` : ''}`)
			.join(', ');
		wiersze.push(`    ustawienia: ${ustawienia}`);
	}

	return wiersze.length > 1 ? wiersze.join('\n') : null;
}
...
export function buildMatchFacts(bundle, { withHeader = true } = {}) {
	...
	if (bundle.form) {
		sections.push(
			'',
			`GOSPODARZE — ${home}`,
			`  ${formatTeamForm(bundle.form.home, 'home')}`,
			'',
			`GOŚCIE — ${away}`,
			`  ${formatTeamForm(bundle.form.away, 'away')}`
		);
	}

	const statsHome = formatTeamStats(bundle.teamStats?.home, home);
	const statsAway = formatTeamStats(bundle.teamStats?.away, away);
	if (statsHome || statsAway) {
		sections.push(
			'',
			'PROFIL SEZONOWY — KIEDY PADAJĄ GOLE I W JAKICH USTAWIENIACH',
			statsHome || `  ${home}: brak danych`,
			statsAway || `  ${away}: brak danych`
		);
	}
