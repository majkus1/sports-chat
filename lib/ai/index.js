...
/**
 * Rozmowa z narzędziami — patrz `runTools` u dostawców. Fallback jak przy reszcie:
 * odmowa nie jest awarią i nie przechodzi do drugiego dostawcy.
 */
export function runTools(options) {
	return withFallback((provider) => provider.runTools(options));
}

/** Wyszukiwanie w internecie — narzędzie asystenta; patrz `lib/assistant/news.js`. */
export function searchWeb(options) {
	return withFallback((provider) => provider.searchWeb(options));
}
